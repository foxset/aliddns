...
#  从本地网卡中获取ipv6地址的前4段
#ipv6s1=`ip addr show eth0 | grep "inet6.*global" | awk '{print $2}' | awk -F"/" '{print $1}'  | tail -n1  | cut -d \: -f 1-4     `  || die "$ipv6"

#  从域名www.foxset.top获取ipv6地址的前4段
ipv6s1=`nslookup -query=AAAA www.foxset.top | grep 'Address: ' | awk '{print $NF}'  | cut -d \: -f 1-4  `     || die "$ipv6" 
