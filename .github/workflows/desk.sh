...
#  从群晖网卡中获取ipv6地址的前4段
#ipv6s=`nslookup -query=AAAA www.foxset.top | grep 'Address: ' | tail -n1 | awk '{print $NF}' | cut -b 1-20` || die "$ipv6" 
#  截取www.foxset.top的ipv6地址的前4段

ipv6s=$ipv6s"d1d7:3e3c:3354:de09"
echo $ipv6s

# 与目标设备固定ipv6的后4段（“d1d7:3e3c:3354:de09”）进行拼接
